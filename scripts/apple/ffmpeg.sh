...
if [[ -z ${FFMPEG_KIT_OPTIMIZED_FOR_SPEED} ]]; then
  SIZE_OPTIONS="--enable-small --disable-everything --enable-videotoolbox --enable-avfoundation --enable-decoder=h264,hevc,aac,mp3,pcm_s16le --enable-encoder=h264_videotoolbox,hevc_videotoolbox,aac --enable-filter=eq,colorbalance,drawtext,sidechaincompress,amix,volume,scale,crop,overlay,concat,format --enable-protocol=file,pipe --enable-demuxer=mov,mp4,m4a,mp3,wav --enable-muxer=mp4,mov,wav"
else
  SIZE_OPTIONS=""
fi
